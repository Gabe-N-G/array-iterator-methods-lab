...
//  testing nodemon console.log ("hello world")
...
// inventors.filter((old) =>{
//     if (old.year < 1599 && old.year > 1500){
//         veryOldInventors.push(old)
//     }
//     return veryOldInventors
// }) 

veryOldInventors = inventors.filter((old) => {
    return old.year < 1599 && old.year > 1500
})
// Check your work:
...
sortedByBirthYear = inventors.sort((inventormin,inventormax) => {
    return inventormin.year - inventormax.year
})
...
firstLast = people.map((person) => {
    let lastFirst= person.split(', ')
    return firstLast = lastFirst[1] + " " + lastFirst[0]
})
...
totalYearsLived = inventors.reduce((accumulator, currentValue) => {
  ...
  // return ((accumulator.passed - accumulator.year) + currentValue, 0)
  // why doesn't accumulator.year work?
},0)
